...
describe('Position', () => {
    ...
    describe('hasBlackPiece', () => {
        it('return true for black piece', () => {
            const position: IPosition = { x: 2, y: 3, isBlack: true };
            ok(hasBlackPiece(position));
        });

        it('return false for white piece', () => {
            const position: IPosition = { x: 2, y: 3, isBlack: false };
            notOk(hasBlackPiece(position));
        });

        describe('return false for no piece', () => {
            it('null', () => {
                const position: IPosition = { x: 2, y: 3, isBlack: null };
                notOk(hasBlackPiece(position));
            });
            it('undefined', () => {
                const position: IPosition = { x: 2, y: 3, isBlack: undefined };
                notOk(hasBlackPiece(position));
            });
            it('no prop', () => {
                const position: IPosition = { x: 2, y: 3 };
