 ...
 * @param p position
 */
var printUnicodeBackgroundBlack = function printUnicodeBackgroundBlack(p) {
    if (hasWhitePiece(p)) return '\u25CF';else if (hasBlackPiece(p)) return '\u25CB';else return ' ';
};
/**
 * Print unicode position for white background.
 * @param p position
 */
var printUnicodeBackgroundWhite = function printUnicodeBackgroundWhite(p) {
    if (hasWhitePiece(p)) return '\u25D9';else if (hasBlackPiece(p)) return '\u25D8';else return '\u2588';
};
/**
 ...
 * @param p position
 */
var printUnicodePosition = function printUnicodePosition(p) {
 ...
 */
var getOrderedPositions = _ramda2.default.curry(function (getYAs, boardSizeY, isBlack, positions) {
 ...
 */
var getOrderedPositionsY0Start = _ramda2.default.curry(getOrderedPositions(getY0Start));
/**
 * Get ordered positions as white IPosition[Y = endRow -> 0][positions]
 */
var getOrderedPositionsY0End = _ramda2.default.curry(getOrderedPositions(getY0End));
